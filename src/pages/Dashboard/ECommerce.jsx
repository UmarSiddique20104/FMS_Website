...
import NotficationModel from './Model';
import useToast from '../../hooks/useToast';
import LoaderGBMSData from '../../common/LoaderGBMSData';
...
const ECommerce = () => {
  ...
  const handleUpdateGBMSDataManually = async (e) => {
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_BASE_URL}/api/v1/activities/syncing-data-gbms`,
        requestOptions,
      );
      ...
      setIsMainLoading(false);
      showErrorToast('Something wents wrong');
      ...
      fuelData.data
        ...
        .forEach((fuel) => {
          ...
          if (statusCounts[status] !== undefined) {
            ...
            statusCounts[status].vehicles.push({
              id: fuel.id, // Include id here
              registrationNo: fuel.registrationNo,
              driverName: fuel.driverName,
              fuelType: fuel.fuelType,
              quantityOfFuel: fuel.quantityOfFuel,
              amount: fuel.amount,
              status: fuel.status,
            });
          }
      ...
      periodicData.data
        ...
        .forEach((periodic) => {
          ...
          if (statusCounts[status] !== undefined) {
            statusCounts[status].count++;
            statusCounts[status].vehicles.push({
              id: periodic.id, // Include id here
    ...
    <table className="table-auto w-full px-2">
      <thead>
        <tr className="bg-gray-200">
          <th className="px-4 py-2 border">S.No</th> {/* Add S.No column */}
      ...
      <tbody>
        {vehicles.map((vehicle, index) => (
          <tr key={index} className="hover:bg-gray-100">
            ...
            <td className="border ps-7 pb-5">
              {' '}
              <span
                onClick={() => NavigateFuelReqDetails(vehicle.id, 'view')}
                className="ps-3 cursor-pointer"
              >
                {' '}
                <FiEye className="text-green-600 h-6 w-6" />
              </span>
            </td>
            {vehicle.status === 'completed' ? (
              <td className="border px-4 py-2 cursor-default">Cannot update</td>
            ) : (
              <td className="border ps-7 pb-5">
                {' '}
                <span
                  onClick={() => NavigateFuelReqDetails(vehicle.id, 'update')}
                  className="ps-3 cursor-pointer"
                >
                  {' '}
                  <HiPencilAlt className="text-green-600 h-6 w-6" />
                </span>
              </td>
            )}
          </tr>
        ...
        {vehicles.map((vehicle, index) => (
          <tr key={index} className="hover:bg-gray-100">
            ...
            <td className="border ps-7 pb-5">
              {' '}
              <span
                onClick={() => NavigateToDetails(vehicle.id, 'view')}
                className="ps-3 cursor-pointer"
              >
                {' '}
                <FiEye className="text-green-600 h-6 w-6" />
            ...
            {vehicle.status === 'completed' ? (
              ...
              <td className="border ps-7 pb-5">
                {' '}
                <span
                  onClick={() => NavigateToDetails(vehicle.id, 'update')}
                  className="ps-3 cursor-pointer"
                >
                  {' '}
                  <HiPencilAlt className="text-green-600 h-6 w-6" />
  ...
  const NavigateToDetails = (id, status) => {
    console.log(status);

    if (data.data && Array.isArray(data.data)) {
      const filterData = data.data.filter((item) => item.id === id);
      if (status === 'update') {
        navigate(`/periodic/update/${id}`);
      } else {
        navigate(`/periodic/view/${id}`);
      }
    } else {
      console.log('Data is not available or in an incorrect format');
    }
  };

  const NavigateFuelReqDetails = (id, status) => {
    if (status === 'update') {
      navigate(`/fuel-management/update/${id}`);
    } else {
      navigate(`/fuel-management/view/${id}`);
    }
  };

  return (
    <>
      {isMainLoading && <LoaderGBMSData />}
      <DefaultLayout>
          ...
          {user?.Role?.roleName === 'companyAdmin' && (
            <div>
              <button
                ...
                onClick={handleUpdateGBMSDataManually}
              >
                Sync GBMS Data
              </button>
            </div>
              ...
              (status, index) => (
                <div key={index} className="flex items-center space-x-2">
                  ...
                  <p className="ml-2 text-sm font-medium text-black dark:text-white">
                    {fuelStatusData.find((data) => data.name === status)
                      ?.value || 0}
                  </p>
                </div>
              ),
        ...
        {isModalOpen && (
          <div className="modal modal-open" style={{ zIndex: 999999 }}>
            <div className="modal-box max-w-4xl relative  xl:min-w-[1100px]  bg-white text-black no-scrollbar">
              <button
