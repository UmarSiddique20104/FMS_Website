...
const Drivers = () => {
  ...
  const exportPDF = (data) => {
    ...
    exportToPDF(data, columnsToFilter, columnsToPrint, 'Drivers');
  };
  ...
  const exportToCsv = (data) => {
    ...
    if (navigator.msSaveBlob) {
      // IE 10+
      navigator.msSaveBlob(blob, 'drivers_data.csv');
    } else {
      ...
      if (link.download !== undefined) {
        ...
        link.setAttribute('download', 'drivers_data.csv');
        ...
        <div className="flex items-end gap-2">
          <div className="dropdown">
            <div
              tabIndex={0}
              role="button"
              className="btn h-[30px] min-h-[30px] text-sm border-slate-200 hover:bg-opacity-70 dark:text-white dark:bg-slate-700 dark:border-slate-700 dark:hover:bg-opacity-70 transition duration-150 ease-in-out bg-primary text-white  rounded-md"
            >
              Export
              <MdDownload />
            </div>
