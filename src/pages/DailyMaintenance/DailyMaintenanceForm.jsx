...
const DailyMaintenanceForm = () => {
  ...
  useEffect(() => {
    if (dailyData && dailyData.data && vehicleData) {
      const filteredData = dailyData.data.filter((dailyItem) =>
        vehicleData.data.some((vehicle) => {
          return (
            vehicle.registrationNo === dailyItem.vehicle.registrationNo &&
            vehicle.station === station.value
          );
        }),
      );
      const addStatus = [...filteredData].map((item) => {
        const vehicle = vehicleData.data.find(
          (vehicle) => vehicle.registrationNo === item.vehicle.registrationNo,
        );
        return {
          ...item,
          vehicle: {
  ...
  const handleVehicleClick = (vehicle, status) => {
