      ...
      {isLoading ? (
        ...
        <>
          ...
          { (EmergencyData?.data?.status === 'survey completed' ||
            ...
            <div className="flex flex-col bg-gray-100 rounded-lg overflow-hidden shadow-lg">
            { EmergencyData?.data?.emergencyMaintenanceInsurance[0] &&  <div className="flex justify-between items-end p-6 border-b border-gray-200">
                <h2 className="text-xl font-bold">Insurance Details</h2>
              </div>}
              <div className="  p-5 bg-brand-primary">
               {  EmergencyData?.data?.emergencyMaintenanceInsurance[0] &&  <InsuranceDetail
                  insuranceData={
                    EmergencyData?.data?.emergencyMaintenanceInsurance[0]
                  }
                />}
                {EmergencyData?.data?.estimatedCost !== '' || EmergencyData?.data?.estimatedCost !== 'undefinend'  && (
                  <DetailItem
