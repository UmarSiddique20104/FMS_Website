...
import { InsuranceDetail, MediaItem } from '../Components/HelperFunction';
...
const CompanyAdminEdit = () => {
  ...
  const [estimatedStatusCost, setEstimatedStatusCost] = useState();
  ...
  useEffect(() => {
    if (EmergencyData) {
      ...
      setFormValues({
        ...
        estimatedCostImage: eData?.estimatedCostImage,
        // services: eData?.services || [],
        status: eData?.status,
      });
      setEstimatedStatusCost(eData);
      setBillDetailsValues({
        ...
        estimatedCost:
          eData?.estimatedCost !== 'undefined' ? eData?.estimatedCost : '',
      });
      setSelectedSupplier({
  ...
  const handleSubmit = async (e) => {
    ...
    if (newErrorMessages.every((msg) => msg === '')) {
      ...
      if (
        billDetailsValues.estimatedCost !== 'undefined' ||
        billDetailsValues.estimatedCost !== ''
      ) {
        updatedFormData.estimatedCost = billDetailsValues.estimatedCost;
        if (
          estimatedStatusCost.estimatedCost !== billDetailsValues.estimatedCost
        ) {
          if (
            estimatedStatusCost.status !== 'approved' &&
            estimatedStatusCost.status !== 'waiting for completion' &&
            estimatedStatusCost.status !== 'completed'
          ) {
            updatedFormData.status = 'survey completed';
            updatedFormData.sendForInsurance = true;
          }
        }
      }
      if (estimatedCostImage.length > 0) {
        updatedFormData.estimatedCostImage = estimatedCostImage;
      }
      ...
      if (
        ...
        EmergencyData?.data?.status == 'statisfiaction note issued'
      ) {
      ...
      console.log('Hello >>>>', EmergencyData?.data);
      if (
        ...
        console.log('Hello >>>>');
        console.log(emergencyRepairCompletionImgUrls);
        console.log(emergencyReceiptImgUrls);
              ...
              <div className="p-7">
                <form action="#" onSubmit={handleSubmit}>
                  ...
                  {(formValues?.status == 'waiting for completion' ||
                    ...
                    formValues?.status == 'inspection done' ||
                    formValues?.status == 'approved') && (
                    <div>
                      <div className=" ">
                        ...
                        <div className="  p-5 bg-brand-primary">
                          {formValues.sendForInsurance === true && (
                            <InsuranceDetail
                              insuranceData={
                                EmergencyData?.data
                                  ?.emergencyMaintenanceInsurance[0]
                              }
                            />
                          )}

                          {EmergencyData?.data?.estimatedCostImage &&
                            EmergencyData?.data?.estimatedCostImage?.length !==
                              0 && (
                              <MediaItem
                        ...
                        <div className="flex  flex-col   gap-5 px-4">
                          <div className="w-1/4 pe-4">
                            <InputField
                              ...
                              type="text"
                              value={billDetailsValues.estimatedCost}
                              onChange={(e) =>
                                setBillDetailsValues({
                                  ...billDetailsValues,
                          ...
                          <div className="w-full pe-10  pb-10">
                            <h2 className="font-bold py-3">
                              {' '}
                              Select the Estimated Cost images
                            </h2>
                            <div className="relative">
                  ...
                  <div className="mb-5.5 flex flex-col gap-5.5 sm:flex-row pt-5">
                    <div className="w-full sm:w-1/2 md:w-1/3">
                      <label
                        className="mb-3 block text-md font-medium text-black dark:text-white"
                    ...
                    {(EmergencyData?.data?.status == 'waiting for completion' ||
                      ...
                      <>
                        <div className="w-full sm:w-1/2 md:w-1/3">
                          ...
                          <div className="relative">
                            {/* {formValues?.emergencyRepairCompletionImgs?.length >
                            0 ? (
                              <ul className="list-disc pl-5">
                                      ...
                                      <li key={url} className="py-2">
                                        ...
                                        >
                                          <img
                                            src={url}
                                            alt={`Emergency Receipt Image ${index + 1}`}
                                            className="object-contain h-48 w-48 border"
                                          />
                                        </a>
                                      </li>
                                    </>
                                              ...
                                              <button
                                                onClick={(e) => {
                                                  ...
                                                  handleDelete(
                                                    index,
                                                    setEmergencyRepairCompletionImgUrls,
                                                  ); // Calls your handleDelete function
                                                }}
                                                className="absolute top-0 right-0 mt-2 mr-2 bg-red-500 text-white rounded-full p-1"
                                              >
                                                &#10005;
